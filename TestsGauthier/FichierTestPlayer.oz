...
define
    ...
    % Retourne variablePasBind#isunboud si la variable n a pas ete bind apres une seconde et false sinon
    % Le comportement de player suite a chaque message est print
    fun {TestRetrieving}
        ...
        local 
            ...
            Answer13
            {Send Port sayPassingDrone(drone(row Position2.x) id(id:992 color:blue name:'Antoine') Answer13)}%un drone passe au dessus de moi en x
            Answer131
            {Send Port sayPassingDrone(drone(column Position2.y) id(id:992 color:blue name:'Antoine') Answer131)}%un drone passe au dessus de moi en y
            Answer132
            {Send Port sayPassingDrone(drone(row Position2.x-1) id(id:992 color:blue name:'Antoine') Answer132)}%un drone passe a coté de moi en x
            Answer133
            {Send Port sayPassingDrone(drone(column Position2.y-1) id(id:992 color:blue name:'Antoine') Answer133)}%un drone passe a coté de moi en y
            Answer14
            {Send Port sayPassingSonar(id(id:992 color:blue name:'Antoine') Answer14)}%reponse doit etre un coordinee vraie l autre fausse
            
            ListOfVariable = [ initPosition ID Position move ID2 Position2 Direction2 chargeItem ID3 KindItem3 fireItem ID4 KindFire4 fireMine ID5 Mine5 isDead Answer6 sayMissileExplode Message11 Message111 Message112 Message113 Message114 sayMineExplode Message12 Message121 Message122 Message123 Message124 sayPassingDrone Answer13 Answer131 Answer132 Answer133 sayPassingSonar Answer14 ]
            
            fun {Loop List}
                case List
                of H|T then
                    {System.show H}
                    if {IsFree H} then H#isunboud
                    else {Loop T}
                    end
                [] nil then false
                end
...
in
    {System.show testRetrieving(error:{TestRetrieving}) }
end
